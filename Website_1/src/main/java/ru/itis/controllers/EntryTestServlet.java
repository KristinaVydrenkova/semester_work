...
@WebServlet("/tests/entryTest")
public class EntryTestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        getServletContext().getRequestDispatcher("/WEB-INF/views/entryTest.jsp").forward(request,response);
    }

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        EntryTestForm form = new EntryTestForm();
        ...
        int scores = testCheckService.checkEntryTest(form,user);
        String result = null;
        if(scores==-1){
